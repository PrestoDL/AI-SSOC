...
#define TRIAL 3        // attempts
#define REDUCTION -0.3  // reduction value
...
void printResult(ppGrid** grid, int x_start, int y_start, int x_goal, int y_goal) {
    for (int j = 0; j < Y_RANGE; j++) {
        for (int i = 0; i < X_RANGE; i++) {
            if (grid[j][i].d == INF) { printf("¡á"); } // print wall

            else if ((i == x_start) && (j == y_start)) { printf("¡Þ"); } // print start point

            else if ((i == x_goal) && (j == y_goal)) { printf("¡Ú"); } // print goal point
                ...
                case 2: { printf("¢×"); break; }
                case 3: { printf("¡æ"); break; }
                case 5: { printf("¡ç"); break; }
                case 6: { printf("¢Ö"); break; }
                case 7: { printf("¡è"); break; }
                case 8: { printf("¢Ø"); break; }
                default: { printf("er"); break; }
                }
            }

            else if (grid[j][i].l == 1) { printf("¤ý"); } // print checked point
...
void printStatus(ppGrid** grid, Queue* agentQueue) {
    ...
    for (int j = 0; j < X_RANGE; j++) {
        for (int i = 0; i < Y_RANGE; i++) {
            if (grid[j][i].d == INF) { printf("¡á"); }
            else if (grid[j][i].l == 4) { printf("¡Þ"); }
            else { printf("  "); }
        }
        printf("\n");
    }
...
Boolean aStar(ppGrid** grid, Agent* agent, Queue* holdQueue, int x_goal, int y_goal, float reductionvalue) {
    for (int j = 0; j < Y_RANGE; j++) {
        for (int i = 0; i < X_RANGE; i++) {
...
void addAgent(Queue* agentQueue, Queue* holdQueue, int x_start, int y_start, int x_goal, int y_goal, float reductionvalue) {
    ...
    Boolean collision;
    do {
        if (aStar(tempGrid, agent, holdQueue, x_goal, y_goal, reductionvalue) == 0) { break; }
...
void main() {
    ...
    for (int tri = 0; tri < TRIAL; tri++) {
        ...
        for (int j = 0; j < 50; j++) {
            ...
            addAgent(agentQueue, holdQueue, sx, sy, gx, gy, 0.1);
        }
        ...
        do {
            ...
            for (int h = 0; h < holdCount; h++) {
                ...
                holdAgent(tempAgent, agentQueue, holdQueue, 0.1);
            }
            //printf("\n<%d agents left>\n", holdCount);
    ...
    for (float red = 0; red < 2.1; red += 0.1) {
        ...
        for (int tri = 0; tri < TRIAL; tri++) {
            ...
            for (int j = 0; j < 50; j++) {
                ...
                addAgent(agentQueue, holdQueue, sx, sy, gx, gy, -red);
            }
